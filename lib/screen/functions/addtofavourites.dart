...
bool checkFavorite(id) {
  List<Songs> dbsongs = box.values.toList();
  Songs song = dbsongs.firstWhere((element) => element.id == id);

  Favourites favValues = Favourites(
      songname: song.songname,
      artist: song.artist,
      duration: song.duration,
      songUrl: song.songUrl,
  ...
  final favoritBox = FavSongBox.getInstance();
  List<Favourites> favoriteDb = favoritBox.values.toList();
  bool isThere = favoriteDb.where((element) => element.id == id).isEmpty;
  return isThere;
}
...
// removeFav(id) async {
//   final favoritBox = FavSongBox.getInstance();
//   List<Favourites> removeFavDb = favoritBox.values.toList();
//   List<Songs> dbSongs = box.values.toList();
//   int currentIndex = removeFavDb.indexWhere((element) => element.id == id);
//   if (currentIndex >= 0) {
//     await  favouritesdb.deleteAt(currentIndex);
//   }
// }
